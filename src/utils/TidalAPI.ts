        ...
        if (data && included && data.length > 0 && included.length > 0) {
            for (let i = 0; i < data.length; i++) {
                ...
                const artists = included
                    .filter(item => item.type == 'artists' && relationships?.artists?.data?.some(artist => artist.id == item.id))
                ...
                const albums = included
                    .filter(item => item.type == 'albums' && relationships?.albums?.data?.some(album => album.id == item.id))
                ...
                if (id && title && link && artists && albums && artists.length > 0 && albums.length > 0)
                    result.push({ id, title, link, artists, album: albums[0] })
            }
        }
